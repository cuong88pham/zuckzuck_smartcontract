...
contract GMPBundle is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function preorder(BundleType bundleType, uint256 quantity) public whenNotPaused notContract{
        ...
        for (uint256 index = 0; index < quantity; index++) {
            ...
            Item memory item = Item({
                ...
                isOpened: false,
                bundleType: bundleType, 
                landSpotSize: NFTLandSlot[bundleType],
                moleQuantity: NFTMoleSlot[bundleType],
                minted_at: block.timestamp
            });
            
            items.push(item); 
            ownerOfBundles[msg.sender][bundleType].push(item);
